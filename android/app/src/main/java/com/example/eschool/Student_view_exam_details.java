...
public class Student_view_exam_details extends AppCompatActivity implements JsonResponse, AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_student_view_exam_details);
        l1=(ListView)findViewById(R.id.lvexam);
        l1.setOnItemClickListener(this);


        JsonReq JR=new JsonReq();
        JR.json_response=(JsonResponse) Student_view_exam_details.this;
        String q = "/student_view_exam_details";
        q=q.replace(" ","%20");
        JR.execute(q);
...
//                }
//            }
            if(method.equalsIgnoreCase("student_view_exam_details")){
                String status=jo.getString("status");
                Log.d("pearl",status);
