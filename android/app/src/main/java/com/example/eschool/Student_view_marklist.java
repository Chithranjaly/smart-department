...
public class Student_view_marklist extends AppCompatActivity  implements JsonResponse, AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_student_view_marklist);
        l1=(ListView)findViewById(R.id.lvmark);
        l1.setOnItemClickListener(this);


        JsonReq JR=new JsonReq();
        JR.json_response=(JsonResponse) Student_view_marklist.this;
        String q = "/student_view_marklist?login_id="+Login.logid;
        q=q.replace(" ","%20");
        JR.execute(q);
...
//                }
//            }
            if(method.equalsIgnoreCase("student_view_marklist")){
                String status=jo.getString("status");
                Log.d("pearl",status);
