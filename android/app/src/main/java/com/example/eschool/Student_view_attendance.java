...
public class Student_view_attendance extends AppCompatActivity  implements JsonResponse, AdapterView.OnItemClickListener  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        JR.json_response=(JsonResponse) Student_view_attendance.this;
        String q = "/student_view_attendance?login_id="+Login.logid;
        q=q.replace(" ","%20");
        JR.execute(q);
...
//                }
//            }
            if(method.equalsIgnoreCase("student_view_attendance")){
                String status=jo.getString("status");
                Log.d("pearl",status);
                ...
                if(status.equalsIgnoreCase("success")){
                    ...
                    for(int i = 0;i<ja1.length();i++)
                    {
                        ...
                        att_class[i]=ja1.getJSONObject(i).getString("att_hour");
